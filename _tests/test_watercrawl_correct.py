...
# -*- coding: utf-8 -*-
"""
WaterCrawl 正しい使用方法テスト

WaterCrawl APIの正しい使用方法をテストするスクリプト
...
def test_watercrawl_correct():
    """WaterCrawlの正しい使用方法をテスト"""
    ...
    try:
        ...
        # 方法1: create_crawl_requestを正しいパラメータで使用
        print("\n📋 方法1: create_crawl_request（正しいパラメータ）")
        try:
        ...
        # 方法2: セッション初期化後にscrape_urlを試行
        print("\n📋 方法2: セッション初期化後にscrape_url")
        try:
            client.init_session()
            print("セッション初期化成功")
        ...
        print("\n📋 方法3: バッチクロールリクエスト")
        try:
            batch_request = client.create_batch_crawl_request(
                urls=[test_url],
                name="Test Batch Crawl"
            )
