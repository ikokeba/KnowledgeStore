...
### process_new_folders.py にフッターテンプレート機能を追加

**タスク**: 各マークダウンファイルの末尾にfooter_template.mdで定義した書式を追加する機能を実装

**実装内容**:
1. `_templates/footer_template.md` ファイルを作成
   - 内容: `*Exported at: {export_date}*` 形式のフッター
   - プレースホルダー `{export_date}` で現在日時を動的置換

2. `process_new_folders.py` に以下の機能を追加:
   - `_load_footer_template()`: フッターテンプレートの読み込みと日時置換
   - `add_footer_to_markdown_files()`: 指定フォルダ内の全.mdファイルにフッター追加
   - 既存フッターの重複チェック機能
   - 処理統計（処理/スキップ/エラー件数）の表示
...
**技術仕様**:
- フッターテンプレートファイル: `_templates/footer_template.md`
- 日時フォーマット: `%m/%d/%Y, %I:%M:%S %p` (例: 09/18/2025, 11:16:33 AM)
- 重複チェック: `*Exported at:` 文字列の存在確認
...
**目的**: コードベース全体を確認し、用途が重複しているスクリプトを整理して、スマートにブックマークを整理できるようにする

**作業内容**:

1. **統合スクリプト作成**: `bookmark_manager.py`
   - すべてのブックマーク処理機能を1つのスクリプトに統合
   - プロパティ追加、フッター追加、重複タグ削除、手動タグ追加機能を統合
   - コマンドライン引数で機能を選択可能
   - ドライラン機能付き

2. **重複スクリプトの削除**:
   - `add_read_property.py` - 統合スクリプトに統合
   - `add_tags_manually.py` - 統合スクリプトに統合
   - `process_single_folder.py` - 統合スクリプトに統合
   - `remove_duplicate_tags.py` - 統合スクリプトに統合

3. **既存スクリプトの更新**:
   - `process_new_folders.py`を統合スクリプトを使用するように更新
   - 重複していたフッター追加機能を削除し、統合スクリプトの機能を呼び出すように変更

4. **ドキュメント更新**:
   - `README.md`を更新して整理後のスクリプト構成を反映
   - 統合スクリプトの使用方法を追加
   - 削除されたスクリプトの説明を削除

**整理後のスクリプト構成**:

- **`bookmark_manager.py`** (推奨): 統合スクリプト
  - `process`: 統合処理（プロパティ追加、フッター追加、重複タグ削除）
  - `property`: プロパティ追加のみ
  - `footer`: フッター追加のみ
  - `dedup`: 重複タグ削除のみ
  - `tags-manual`: 手動タグ追加のみ（キーワードベース、API不要）
  - `list`: 利用可能なディレクトリ一覧表示

- **`tag_generator.py`**: OpenAI APIを使用した高品質タグ自動生成
  - API使用料が発生するが、高品質なタグを生成

- **`process_new_folders.py`**: 新規フォルダ自動検出・処理
  - 新規フォルダを自動検出
  - タグ生成、プロパティ追加、フッター追加を一括実行
  - 処理済みフォルダの記録機能

**メリット**:
- スクリプトの重複を解消し、保守性が向上
- 1つのスクリプトで全機能を利用可能
- コマンドライン引数で柔軟に機能を選択可能
- ドライラン機能で安全にテスト可能
...
**目的**: ブックマーク内のゲーム関連情報を収集し、`game.md`に整理

**作業内容**:
- ブックマークフォルダ内からゲーム関連のファイルを検索
- ゲーム関連の情報をカテゴリ別に分類・整理
- `game.md`ファイルを作成して情報をまとめ

**収集した情報**:
1. **プログラミング学習ゲーム**
   - 「農家は Replace() されました」: Python風プログラミング言語で農業を全自動化するゲーム
   - 農業自動化ゲームの実装例

2. **ゲーム開発ツール・技術**
   - GPT-4oを使った3Dワールド生成
   - Minecraft Countries by GPT-4o
   - スポーツアイコンの3D生成

3. **シミュレーションゲーム**
   - リアルタイム進化シミュレーター（デジタルオーガニズム）

**成果物**:
- `game.md`: ゲーム関連情報をカテゴリ別に整理したサマリーファイル
  - 各情報の日付、作者、URL、概要、特徴を記載
  - 技術スタック、参考リンクを含む

**分類方法**:
- プログラミング学習ゲーム: 学習目的のゲーム
- ゲーム開発ツール・技術: AIを活用したゲーム開発技術
- シミュレーションゲーム: シミュレーション系のゲーム

**次のステップ**:
1. ゲーム関連の新規ブックマーク追加時に`game.md`を更新
2. 必要に応じてカテゴリの追加・再分類